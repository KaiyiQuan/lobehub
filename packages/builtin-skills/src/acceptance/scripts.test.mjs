...
// The helpers run in an unrelated project, with no ancestor node_modules and no
// executable bits, exactly like files materialized by `lh acceptance install`.
function project() {
  ...
  for (const [relative, resource] of Object.entries(AcceptanceSkill.resources)) {
    const destination = path.join(root, '.agents/skills/acceptance', relative);
    mkdirSync(path.dirname(destination), { recursive: true });
    writeFileSync(destination, resource.content, { mode: 0o644 });
  }
  return { root, scripts };
}
...
async function cdp(mode = 'capture') {
  const calls = [];
      ...
      while (buffered.length >= 6) {
        ...
        const result =
          message.method === 'Page.getLayoutMetrics'
            ...
            : { data: (mode === 'invalid-png' ? Buffer.from('invalid') : png).toString('base64') };
...
function toolPath(
  ...
  const bmp = Buffer.alloc(offset + 16 * 16 * (rgba ? 4 : 3), brightness);
  bmp.fill(0, 0, offset);
  bmp.write('BM');
  ...
  return { PATH: bin, FIXTURE_ROOT: root, TERM_PROGRAM: 'Test "Terminal"' };
}
...
test('captures the specified target, including full-page bounds, outside any repository', async () => {
  ...
  assert.deepEqual(calls[1].params.clip, { x: 0, y: 0, width: 801, height: 1801, scale: 1 });
...
test('reports an unavailable brightness check instead of PASS', async () => {
  const { root, scripts } = project();
  const { port } = await cdp();
  const out = path.join(root, 'kept.png');
  const result = await run(
    path.join(scripts, 'cdp-screenshot.sh'),
    ['--port', port, '--check', '--out', out],
    toolPath(root, { missing: ['sips'] }),
  );
  assert.equal(result.code, 2);
  assert.doesNotMatch(result.stdout, /PREFLIGHT PASS/);
  assert.deepEqual(readFileSync(out), png);
});

for (const brightness of [0, 255]) {
  test(`CDP preflight measures ${brightness === 0 ? 'black' : 'live'} frames and preserves explicit output`, async () => {
    const { root, scripts } = project();
    const { port } = await cdp();
    const out = path.join(root, 'kept.png');
    const result = await run(
      path.join(scripts, 'cdp-screenshot.sh'),
      ['--port', port, '--check', '--out', out],
      toolPath(root, { brightness }),
    );
    assert.equal(result.code, brightness === 0 ? 6 : 0, result.stdout + result.stderr);
    assert.deepEqual(readFileSync(out), png);
  });
}
...
for (const [scenario, options, code, permission, capture] of [
  ...
  ['black capture', { brightness: 0 }, 4, 'granted', 'black'],
